...
kernel void st_row_reduce(gts_kern_PARAMS(T))
{
    ...
    row_max(vec_a, shared_A, simd_lane_id);
    zero(shared_A, simd_lane_id);
    add_row(shared_A, shared_A, vec_a, simd_lane_id);
    store(output, shared_A, {0,0,0,0}, simd_lane_id);
}
    
template<typename T, unsigned H, unsigned W>
kernel void st_col_reduce(gts_kern_PARAMS(T))
{
    using shared_tile = st<T, H * 8, W * 8>;
    ...
    GL output(_output, nullptr, nullptr, nullptr, nullptr);
    load(shared_A, input, {0,0,0,0}, simd_lane_id);
    col_max(vec_a, shared_A, simd_lane_id);
    zero(shared_A, simd_lane_id);
    add_col(shared_A, shared_A, vec_a, simd_lane_id);
    store(output, shared_A, {0,0,0,0}, simd_lane_id);
}
...
#define gen_kernel(reg_N, reg_M) \
    template [[host_name("st_row_max_float_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_row_reduce<float, reg_N, reg_M>(gts_kern_PARAMS(float)); \
    template [[host_name("st_row_max_bf16_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_row_reduce<bf16, reg_N, reg_M>(gts_kern_PARAMS(bf16)); \
    template [[host_name("st_row_max_half_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_row_reduce<half, reg_N, reg_M>(gts_kern_PARAMS(half)); \
\
    template [[host_name("st_col_max_float_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_col_reduce<float, reg_N, reg_M>(gts_kern_PARAMS(float)); \
    template [[host_name("st_col_max_bf16_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_col_reduce<bf16, reg_N, reg_M>(gts_kern_PARAMS(bf16)); \
    template [[host_name("st_col_max_half_" #reg_N "x" #reg_M)]] [[kernel]] \
    void st_col_reduce<half, reg_N, reg_M>(gts_kern_PARAMS(half)); \
    \
