...
kernel void sv_copy(gts_kern_PARAMS(T))
{
...
#define gen_kernels(reg_N) \
    template [[host_name("sv_copy_float_" #reg_N)]] [[kernel]] \
    void sv_copy<float, reg_N>(gts_kern_PARAMS(float)); \
    template [[host_name("sv_copy_bf16_" #reg_N)]] [[kernel]] \
    void sv_copy<bf16, reg_N>(gts_kern_PARAMS(bf16)); \
    template [[host_name("sv_copy_half_" #reg_N)]] [[kernel]] \
    void sv_copy<half, reg_N>(gts_kern_PARAMS(half)); \
