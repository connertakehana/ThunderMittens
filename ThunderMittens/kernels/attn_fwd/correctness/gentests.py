...
with torch.backends.cpu.sdp_kernel(
    ...
    q = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu').requires_grad_()
    k = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu').requires_grad_()
    v = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu').requires_grad_()
    ...
    # Prepare for timing
    start_events = [torch.cpu.Event(enable_timing=True) for _ in range(30)]
    end_events = [torch.cpu.Event(enable_timing=True) for _ in range(30)]
...
with torch.backends.cpu.sdp_kernel(
    ...
    k = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu').requires_grad_()
    v = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu').requires_grad_()
    grad_output = torch.randn((B, H, N, D), dtype=torch.float16, device='cpu')
